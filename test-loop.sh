...
GITREPO_NAME="${1:?Usage: $0 GITREPO_NAME NAMESPACE MAX_HEAD_DELAY_MS HOST PORT WEBHOOK_TARGET [VENDOR] [OWNER] [REPO] [BRANCH] [SECRET]}"
NAMESPACE="${2:?Usage: $0 GITREPO_NAME NAMESPACE MAX_HEAD_DELAY_MS HOST PORT WEBHOOK_TARGET [VENDOR] [OWNER] [REPO] [BRANCH] [SECRET]}"
MAX_HEAD_DELAY_MS="${3:-0}"
HOST="${4:-localhost}"
PORT="${5:-8080}"
WEBHOOK_TARGET="${6:?Usage: $0 GITREPO_NAME NAMESPACE MAX_HEAD_DELAY_MS HOST PORT WEBHOOK_TARGET [VENDOR] [OWNER] [REPO] [BRANCH] [SECRET]}"
VENDOR="${7:-github}"
OWNER="${8:-test-org}"
REPO="${9:-test-repo}"
BRANCH="${10:-main}"
...
while true; do
  ...
  if [[ "${MAX_HEAD_DELAY_MS}" -gt 0 ]]; then
    HEAD_DELAY_MS=$(( RANDOM % (MAX_HEAD_DELAY_MS + 1) ))
  else
    HEAD_DELAY_MS=0
  fi
  echo ""
